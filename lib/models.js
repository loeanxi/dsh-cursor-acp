...
export const CURSOR_EFFORTS = ['low', 'medium', 'high', 'xhigh', 'max'];
...
export function parseEffort(value) {
    if (typeof value === 'string' && CURSOR_EFFORTS.includes(value)) {
        return value;
    }
    return 'high';
}
 ...
 */
export function familyLabelFrom(label) {
    return label
        .replace(/\s+Fast$/iu, '')
 ...
 */
export function splitCursorModelId(id) {
    if (id === 'auto')
        return { family: 'auto', fast: false };
    let rest = id;
    let fast = false;
...
export function parseCursorModels(stdout) {
    ...
    for (const line of stdout.split(/\r?\n/)) {
        ...
        if (trimmed === '' || trimmed.startsWith('Available') || trimmed.startsWith('Tip:'))
            continue;
    ...
    if (!seen.has('auto'))
        rows.unshift({ id: 'auto', label: 'Auto' });
    return rows;
}
/**
 ...
 */
export function catalogFromOptions(options) {
 ...
 */
export function clampEffort(wanted, available) {
    if (available.length === 0)
        return wanted;
    if (available.includes(wanted))
        return wanted;
    for (const fallback of ['high', 'medium', 'low', 'xhigh', 'max']) {
 ...
 */
export function composeCursorModelId(choice, options) {
    ...
    if (family === undefined && mine.length === 0)
        return model;
 ...
 */
export function withModelArgs(args, model) {
    const cleaned = [];
    ...
    if (acpAt === -1)
        return [...cleaned, '--model', normalized];
