window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function choiceFrom(scope, status) {
			const value = scope.getSnapshot().value || {};
			if (status !== "loading" && status !== "error") {
				return {
					model: value.model || status.model,
					effort: value.effort || status.effort,
					fast: value.fast === true || (value.fast === undefined && status.fast === true),
				};
			}
			return { model: value.model || "auto", effort: value.effort || "high", fast: value.fast === true };
		}
		...
		function CursorAcpSection(props) {
			...
			React.useEffect(() => settingsScope.subscribe(() => setTick((n) => n + 1)), [settingsScope]);
			...
			const applyChoice = (next) => {
				void (async () => {
					...
					setDraft(null);
					try { setStatus(await loadStatus()); } catch { setStatus("error"); }
				})();
			};
			return jsxs("section", {
				...
				children: [
					...
					status !== "loading" && status !== "error" ? jsxs(Fragment, { children: [
						...
						writable ? null : jsx("p", { style: { marginTop: 8 }, children: t("modelUnavailable") }),
		...
		function apply(ctx) {
			...
			const settingsScope = ctx.settingsScope.bind({
				...
				decode: (section) => {
					...
					if (typeof section.model !== "string") return undefined;
					return {
						model: section.model,
						effort: typeof section.effort === "string" ? section.effort : "high",
						fast: section.fast === true,
					};
				},
			});
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				label: () => t("nav"),
				locale: NS,
				inject: () => ({ t: (key) => t(key), loadStatus, settingsScope }),
			}, CursorAcpSection));
		}
		exports.apply = apply;
