window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const warnText = { color: "var(--dsw-alias-danger, #c00)" };
			...
			return jsxs("section", {
				...
				children: [
					...
					status !== "loading" && status !== "error" ? jsxs(Fragment, { children: [
						...
						status.found ? jsxs(Fragment, { children: [
							jsx("p", { style: status.login === "signed-in" ? undefined : warnText, children: status.login === "signed-in" ? t("signedIn") : status.login === "signed-out" ? t("signedOut") : t("loginUnknown") }),
							status.proxy === "set" || status.login === "signed-in" ? null : jsx("p", { style: warnText, children: status.proxy === "partial" ? t("proxyPartial") : t("proxyMissing") }),
						...
						status.found ? null : jsx("p", { children: t(typeof navigator !== "undefined" && /Win/i.test(navigator.platform) ? "installWin" : "installUnix") }),
					] }) : null,
				],
			});
		}
