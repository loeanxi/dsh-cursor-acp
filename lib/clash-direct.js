 ...
 */
export function clashDirectsNode(text) {
    for (const line of text.split(/\r?\n/)) {
        const trimmed = line.trim();
        if (trimmed === '' || trimmed.startsWith('#'))
            continue;
        if (NODE_DIRECT.test(trimmed))
            return true;
    }
    return false;
}
 ...
 */
export function clashConfigCandidates(env, home) {
 ...
 * @param home - user home.
 */
export function detectClashNodeDirect(env = process.env, home = homedir()) {
