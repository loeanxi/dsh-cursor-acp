...
import { spawnAgentEnv } from './proxy-env.js';
import { probeAgentLogin } from './readiness.js';
 ...
 */
export function argsForProbeCommand(args, model, workspace) {
 ...
 */
export function parseProbeOutput(stdout, stderr, exitCode) {
    if (exitCode === null)
        return { kind: 'timeout' };
    if (/\bpong\b/iu.test(stdout))
        return { kind: 'ok' };
    const text = `${stdout}\n${stderr}`;
 ...
 */
export function runCursorProbe(command, args, model) {
