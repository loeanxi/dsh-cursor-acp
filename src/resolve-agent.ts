...
export type AgentSource = 'path' | 'localappdata' | 'home-local'

/** Spawn argv for `agent acp`. */
 ...
 */
export function isSpawnableAgent(path: string): boolean {
  if (!existsSync(path)) return false
  if (process.platform === 'win32') return path.toLowerCase().endsWith('.exe')
  return true
}
 ...
 */
export function versionSortKey(name: string): number {
 ...
 */
export function resolveNodeAgentBundle(root: string): { command: string; args: string[] } | undefined {
 ...
 */
export function searchPathForAgent(pathEnv: string | undefined): string | undefined {
  ...
  for (const dir of pathEnv.split(delimiter)) {
    ...
    for (const name of names) {
      ...
      if (isSpawnableAgent(candidate)) return candidate
    }
  }
  return undefined
}
 ...
 * @param pathEnv - `process.env.PATH`.
 */
export function searchPathForAgentCmd(pathEnv: string | undefined): string | undefined {
  if (pathEnv === undefined || pathEnv === '') return undefined
  for (const dir of pathEnv.split(delimiter)) {
    if (dir === '') continue
    const cmd = join(dir, 'agent.cmd')
    if (existsSync(cmd)) return dirname(cmd)
  }
  return undefined
}
 ...
 */
export function wellKnownAgentPaths(env: NodeJS.ProcessEnv): readonly { path: string; source: AgentSource }[] {
  const home = env.HOME ?? env.USERPROFILE ?? homedir()
  const local = env.LOCALAPPDATA
  const found: { path: string; source: AgentSource }[] = []
 ...
 */
export function resolveAgent(env: NodeJS.ProcessEnv = process.env): ResolvedAgent | undefined {
  ...
  if (cmdRoot !== undefined) {
    ...
    if (bundle !== undefined) return { ...bundle, source: 'path' }
  }
  const local = env.LOCALAPPDATA
  if (local !== undefined && local !== '') {
    const bundle = resolveNodeAgentBundle(join(local, 'cursor-agent'))
    if (bundle !== undefined) return { ...bundle, source: 'localappdata' }
  }
