...
/** Whether `agent status` says this machine is signed in. */
export type LoginState = 'signed-in' | 'signed-out' | 'unknown'

/** Whether this process has proxy env that Node will actually use. */
export type ProxyKind = 'missing' | 'partial' | 'set'

const PROXY_NAMES = ['HTTP_PROXY', 'HTTPS_PROXY', 'ALL_PROXY', 'http_proxy', 'https_proxy', 'all_proxy'] as const

/**
 * Names of proxy variables that are set. Values are dropped on purpose.
 * @param env - process env.
 */
export function readProxyEnv(env: NodeJS.ProcessEnv): { kind: ProxyKind; names: readonly string[] } {
  const names = PROXY_NAMES.filter((name) => {
    const value = env[name]
    return value !== undefined && value.trim() !== ''
  })
  const nodeFlag = env.NODE_USE_ENV_PROXY === '1'
  if (names.length === 0) return { kind: 'missing', names: [] }
  if (!nodeFlag) return { kind: 'partial', names }
 ...
 */
export function parseAgentStatus(stdout: string, stderr: string, exitCode: number | null): LoginState {
  const fromJson = parseStatusJson(stdout)
  if (fromJson !== undefined) return fromJson
  const text = `${stdout}\n${stderr}`
  if (/logged in as/iu.test(text) || /✓\s*logged in/iu.test(text)) return 'signed-in'
  if (
  ...
  if (exitCode === 0 && stdout.trim() !== '') return 'unknown'
  return 'unknown'
}
 ...
 */
export function probeAgentLogin(command: string, args: readonly string[]): LoginState {
...
function parseStatusJson(stdout: string): LoginState | undefined {
  const trimmed = stdout.trim()
  if (trimmed === '' || !trimmed.startsWith('{')) return undefined
  try {
    const raw: unknown = JSON.parse(trimmed)
    if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) return undefined
    const rec = raw as { isAuthenticated?: unknown; status?: unknown }
    if (rec.isAuthenticated === true || rec.status === 'authenticated') return 'signed-in'
    if (rec.isAuthenticated === false) return 'signed-out'
